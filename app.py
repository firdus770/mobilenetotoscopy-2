...
def build_model(num_classes: int) -> nn.Module:
    # Avoid downloading ImageNet weights; your checkpoint already has weights.
...
def load_model_and_classes(weights_path: str):
    ...
    # strict load (will fail if head shape mismatch)
...
with st.sidebar:
    ...
    # Optional speed tweak on CPU-only hosts:
...
if uploads:
    ...
    for up in uploads:
        ...
        top_items = sorted(probs_dict.items(), key=lambda kv: kv[1], reverse=True)[:top_k]
        table = {"class": [k for k,_ in top_items], "probability": [float(v) for _,v in top_items]}
